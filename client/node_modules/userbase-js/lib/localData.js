...
const getCurrentSession = tryCatchWrapper(() => {
  ...
  if (sessionStorageCurrentSessionString) {
    ...
    if (!currentSession.signedIn) {
      ...
      if (localCurrentSessionString) {
        ...
        if (localCurrentSession.signedIn) {
          return _objectSpread({}, localCurrentSession, {
            rememberMe: 'local'
          });
        }
      }
    ...
    return _objectSpread({}, JSON.parse(sessionStorageCurrentSessionString), {
      rememberMe: 'session'
    });
  }

  const localSessionString = localStorage.getItem('userbaseCurrentSession');
  return localSessionString && _objectSpread({}, JSON.parse(localSessionString), {
    rememberMe: 'local'
  });
});
