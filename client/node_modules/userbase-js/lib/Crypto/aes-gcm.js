...
const importKeyFromMaster = async (masterKey, salt) => {
  const encryptionKey = await window.crypto.subtle.deriveKey(hkdf.getParams(ENCRYPTION_KEY_NAME, salt), masterKey, getEncryptionKeyParams(), KEY_IS_EXTRACTABLE, KEY_WILL_BE_USED_TO);
  return encryptionKey;
};

const generateKey = async () => {
  const key = await window.crypto.subtle.generateKey(getEncryptionKeyParams(), KEY_IS_EXTRACTABLE, KEY_WILL_BE_USED_TO);
  return key;
};
...
const getKeyFromRawKey = async rawKey => {
  const key = await windowOrSelfObject().crypto.subtle.importKey(RAW_KEY_TYPE, rawKey, {
    name: ENCRYPTION_ALGORITHM_NAME
  }, KEY_IS_EXTRACTABLE, KEY_WILL_BE_USED_TO);
  return key;
};
...
const getPasswordBasedEncryptionKey = async (hkdfKey, salt) => {
  const encryptionKey = await window.crypto.subtle.deriveKey(hkdf.getParams(PASSWORD_BASED_ENCRYPTION_KEY, salt), hkdfKey, getEncryptionKeyParams(), !KEY_IS_EXTRACTABLE, KEY_WILL_BE_USED_TO);
  return encryptionKey;
};
