...
const signIn = async params => {
  try {
    ...
    const session = _objectSpread({}, apiSignInResult.session, {
      username
    });
...
const signInWithSession = async appId => {
  try {
    ...
    const session = _objectSpread({}, currentSession, {
      authToken
    });
...
const _buildUpdateUserParams = async params => {
  ...
  if (params.newPassword) {
    const [currentPasswordSalts, newPasswordPromise] = await Promise.all([_getPasswordSaltsOverWebSocket(), _generatePasswordToken(params.newPassword, base64.decode(ws.seedString))]); // current password
...
const updateUser = async params => {
  try {
    ...
    const finalParams = await _buildUpdateUserParams(_objectSpread({}, params));
    if (ws.reconnecting) throw new errors.Reconnecting();
    if (!ws.keys.init) throw new errors.UserNotSignedIn(); // ensures same user still attempting to update (seed should remain constant)
...
const forgotPassword = async params => {
  try {
    ...
    const forgotPasswordWs = new WebSocket("".concat(getWsUrl(config.getEndpoint()), "/api/auth/forgot-password?appId=").concat(appId, "&username=").concat(username));
